...
MOCK_DIM = 384  # match MiniLM output size, but value doesn't matter for tests


class FakeEmbedFn(EmbeddingFunction):
    """Deterministic offline embedder so tests don't load the ONNX model
    or hit any network. Each input maps to a constant vector — fine for
    exercising the indexing/retrieval glue."""

    def __call__(self, input: Documents) -> Embeddings:
        return [[0.1] * MOCK_DIM for _ in input]


def _make_rag() -> RAGSystem:
    return RAGSystem(embed_fn=FakeEmbedFn())
